...
// ── Sprint 4 — Agentes públicos ────────────────────────────────────────

export async function fetchPublicAgents(params = {}) {
  const res = await fetch(buildUrl("/agents", params));
  if (!res.ok) return { data: [], meta: { total: 0, page: 1, perPage: 20, totalPages: 0 } };
  return res.json();
}

export async function fetchPublicAgentBySlug(slug) {
  const res = await fetch(buildUrl(`/agents/${encodeURIComponent(slug)}`));
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`Error al cargar agente ${slug}: ${res.status}`);
  return res.json();
}
